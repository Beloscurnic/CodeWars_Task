namespace CodeWars_Task
{
    ...
    //Вывод будет возвращен как Dictionary<string, List<int>> с двумя парами ключ-значение: "pos"и "peaks".
    //Если в данном массиве нет пика, просто верните {"pos" => new List<int>(), "peaks" => new List<int>()
    //}.

    //Пример: pickPeaks([3, 2, 3, 6, 4, 1, 2, 3, 2, 1, 2, 3])должно возвращаться { pos: [3, 7], peaks: [6, 3]}
    ...
    //что находится после, а что до, и, следовательно, мы не знаем, пик это или нет).

    //Также остерегайтесь плато!!! [1, 2, 2, 2, 1] имеет пик в то время как [1, 2, 2, 2, 3] и[1, 2, 2, 2, 2]нет.
    //В случае плато-пика, пожалуйста, верните только положение и значение начала плато.
    //Например: pickPeaks([1, 2, 2, 2, 1])возвращает
