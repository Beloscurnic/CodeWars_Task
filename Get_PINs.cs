...
{
    //    Итак, детектив, один из наших коллег успешно обнаружил нашу цель, грабителя Робби.Следуем за ним на секретный склад,
    //    где предполагаем найти все украденное.Дверь в этот склад защищена электронным кодовым замком.
    ...
    //    │ 0 │
    //    └───┘
    //Он отметил PIN-код 1357, но также сказал, что возможно, что каждая из цифр, которые он видел,
    //на самом деле может быть другой соседней цифрой (по горизонтали или вертикали, но не по диагонали).
    //Например, вместо этого 1 также может быть 2 или 4. А вместо, 5это может быть 2, 4, 6или 8.

    //Он также упомянул, что знает такие замки.Вы можете ввести неограниченное количество неправильных PIN-кодов,
    //они никогда не заблокируют систему окончательно и не подадут сигнал тревоги.
    ...
    //которая возвращает массив (или список в Java/Kotlin и C#) всех вариантов наблюдаемого PIN-кода длиной от 1 до 8 цифр.
    //Мы могли бы назвать функцию getPINs( get_pinsв python, GetPINsв C#). Но обратите внимание, что все PIN-коды,
    //как наблюдаемые, так и результаты, должны быть строками из-за потенциально ведущих нулей.
